...
class IsotermaAdsorcao:
    ...
    def plot_graph(self):
        ...
        if self.model_name == "Freundlich":
            ...
            ax.plot(self.df['log_C'], self.results['slope'] * self.df['log_C'] + self.results['intercept'], 
                    'k--', label=f"Freundlich ($R^2={self.results['R2']:.4f}$)")
            ax.set_xlabel('log C')
            ...
            ax.plot(self.df['Conc_Final'], self.results['slope'] * self.df['Conc_Final'] + self.results['intercept'], 
                    'k--', label=f"Langmuir ($R^2={self.results['R2']:.4f}$)")
