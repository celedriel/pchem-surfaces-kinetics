...
class IsotermaAdsorcao(BaseAnalise):
    ...
    def process_data(self) -> pd.DataFrame:
        ...
        self.df = self.df[
            (self.df["Vol_Aliquota"] > 0) & (self.df["Massa_Adsorvente"] > 0)
        ].copy()

        self.df["Conc_Final"] = (
            self.df["Vol_Gasto"] * self.titrant_conc * self.fator_esteq
        ) / self.df["Vol_Aliquota"]

        self.df["X_Adsorvido"] = (
            (self.df["Conc_Inicial"] - self.df["Conc_Final"])
            * (self.df["Vol_Total"] / 1000)
            * self.molar_mass
        )
    ...
    def fit_model(self, model_name: str) -> Dict[str, Any]:
        if len(self.df) < 3:
            return {
                "erro": "Insuficiência de dados: o modelo exige pelo menos 3 pontos experimentais válidos após a filtragem."
            }
        ...
        try:
            if model_name == "Freundlich":
                ...
                if n_val is None:
                    self.results["aviso"] = (
                        "Atenção: A inclinação da reta de Freundlich é zero, resultando em 'n' indefinido."
                    )
                ...
                k_l = (
                    ...
                    if (reg.intercept != 0 and reg.slope > 0)
                    else None
                )
                ...
                if reg.slope <= 0:
                    self.results["aviso"] = (
                        "Atenção: A inclinação é nula ou negativa. O modelo de Langmuir não é aplicável e q_max foi invalidado."
                    )
                elif abs(reg.intercept) < 1e-10:
                    self.results["aviso"] = (
                        "Intercepto próximo de zero. Verifique a validade do modelo de Langmuir."
                    )
    ...
    def plot_graph(self, model_name: str):
        ...
        if model_name == "Freundlich":
            ax.plot(
                self.df["log_C"], self.df["log_xm"], "bo", label="Dados Experimentais"
            )
            ax.plot(
                self.df["log_C"],
                self.results.get("slope", 0) * self.df["log_C"]
                + self.results.get("intercept", 0),
                "k--",
                label=f"Freundlich ($R^2={self.results.get('R2', 0):.4f}$)",
            )
            ax.set_xlabel("log C")
            ...
            ax.plot(
                self.df["Conc_Final"],
                self.results.get("slope", 0) * self.df["Conc_Final"]
                + self.results.get("intercept", 0),
                "k--",
                label=f"Langmuir ($R^2={self.results.get('R2', 0):.4f}$)",
            )
