...
class OrdemReacao(BaseAnalise):
    ...
    def fit_model(self) -> Dict[str, Any]:
        ...
        if "1a_ordem" not in self.results and "2a_ordem" not in self.results:
            return {
                "erro": f"Falha na convergência matemática geral. Detalhes: {' | '.join(erros_convergencia)}"
            }

        if "1a_ordem" in self.results and "2a_ordem" in self.results:
            self.results["best_fit"] = (
                "1ª Ordem"
                if self.results["1a_ordem"]["MSE"] < self.results["2a_ordem"]["MSE"]
                else "2ª Ordem"
            )
        elif "1a_ordem" in self.results:
            self.results["best_fit"] = "1ª Ordem (2ª Ordem falhou na convergência)"
        else:
            self.results["best_fit"] = "2ª Ordem (1ª Ordem falhou na convergência)"
    ...
    def plot_graph(self):
        ...
        if "1a_ordem" in self.results:
            c_fit1 = func_1a_ordem(
                t_smooth, self.results["1a_ordem"]["C0"], self.results["1a_ordem"]["k"]
            )
            ...
            ax1.set_title("Ajuste Não-Linear: 1ª Ordem (Falha)")
        ...
        if "2a_ordem" in self.results:
            c_fit2 = func_2a_ordem(
                t_smooth, self.results["2a_ordem"]["C0"], self.results["2a_ordem"]["k"]
            )
            ...
            ax2.set_title("Ajuste Não-Linear: 2ª Ordem (Falha)")
