...
class CineticaArrhenius(BaseAnalise):
    ...
    def process_data(self) -> pd.DataFrame:
        ...
        self.df = self.df[
            (self.df["Temperatura (K)"] > 0) & (self.df["Tempo (s)"] > 0)
        ].copy()
    ...
    def fit_model(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if reg.slope > 0:
                self.results["aviso"] = (
                    "Atenção: A inclinação (slope) é positiva, resultando em uma Energia de Ativação negativa. Verifique os dados experimentais."
                )
    ...
    def plot_graph(self):
        ...
        ax.plot(
            self.df["1/T"],
            self.results.get("slope", 0) * self.df["1/T"]
            + self.results.get("intercept", 0),
            "k--",
            label=f"Arrhenius ($R^2={self.results.get('R2', 0):.4f}$)",
        )
